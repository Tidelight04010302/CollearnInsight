...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QTabWidget, QWidget, QSplitter,
                             ...
                             QMessageBox,QTabBar,QSpinBox,QInputDialog)
...
from sklearn.cluster import AgglomerativeClustering  # 导入AgglomerativeClustering用于层次聚类
from sklearn.preprocessing import StandardScaler  # 导入StandardScaler用于数据标准化
from scipy.cluster.hierarchy import dendrogram, linkage  # 导入dendrogram和linkage用于绘制树状图
...
class CombinedApplication(QMainWindow):
    def __init__(self):
        ...
        self.video_player.setMaximumWidth(950)
        self.data_encoder.setMinimumWidth(950)
        self.data_encoder.setMaximumWidth(1250)
        self.cluster_analysis.setMinimumWidth(950)
        ...
        main_layout.addWidget(tab_module_container)
        ...
        for page_idx, page in enumerate(data_pages):
            ...
            for row_idx in range(row_count):
                ...
                for col_idx in range(table.columnCount()):
                    ...
                    new_header = f"S{page_idx + 1}_Eoding{current_dim_index}"
...
class DataEncoderModule(QWidget):
    ...
    def output(self):
        columns = [module.name for module in self.modules] + ["注释"]
        if not self.selected_data:
            df = pd.DataFrame(columns=["时间"] + columns)
        else:
            df = pd.DataFrame(self.selected_data, columns=columns)
            try:
                time_unit = int(self.time_unit_entry.text())
            except:
                time_unit = 1
            # 获取起始时间
            try:
                start_time = int(self.startmin.text() or 0) * 60 + int(self.startsec.text() or 0)
            except ValueError:
                start_time = 0
            
        
            # 生成格式化时间列表
            formatted_times = [self.format_seconds(start_time + i * time_unit) for i in range(len(df))]
        
            # 插入起始时间列
            df.insert(0, '时间', formatted_times)
        ...
        if file_path:
            with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                df.to_excel(writer, sheet_name='数据', index=False, startrow=1, header=True)
...
class ClusterAnalysisModule(QWidget):
    ...
    def display_results(self, results):
        ...
        for row, result in enumerate(results):
            ...
            # 组别
            self.result_table.setItem(row, 1, QTableWidgetItem(str(result['Group'])))
            
            # 维度结果
            for col in range(2, min(len(result), 5)):
                self.result_table.setItem(row, col, QTableWidgetItem(str(result[f'Dim{col-1}'])))
...
class SequenceAnalysisModule(QWidget):
    ...
    def select_file(self):
        ...
        try:
            ...
            if time_column is None:
                raise ValueError("时间列不存在（文件中没有列）")
            if not isinstance(time_column, str) or "时间" not in time_column:
                raise ValueError(f"第一列 '{time_column}' 不是时间列（应包含 'Time' 或 '时间' 字样）")
    ...
    def plot_data(self, file_path):
        try:
            ...
            for ax_idx, dim in enumerate(dims):
                ...
                # ✅ 修改：仅当 y_positions 非空时才绘制白色间隙背景
                if y_positions:
