...
class CombinedApplication(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("学习行为编码与分析系统")
        ...
        self.data_encoder = DataEncoderModule(tabmodule=self.tabmodule_widget)
        ...
        # 在 top_layout.addWidget(self.module_edit) 之后添加：
    ...
    def get_archive_data(self):
        """构建存档数据"""
        archive_data = {
            "video": self.video_player.current_file_path,
            "tabs": [],
            "summary": []
        }
        ...
        for i in range(self.tabmodule_widget.count()):
            widget = self.tabmodule_widget.widget(i)
            if isinstance(widget, DataEncoderModule) and widget is not self.summary_tab:
                tab_data = {
                    "name": self.tabmodule_widget.tabText(i),
                    "data": widget.to_dict()
                }
                archive_data["tabs"].append(tab_data)

        # 保存汇总数据
        summary_data = []
        for row in range(self.summary_tab.summary_table.rowCount()):
            row_data = []
            for col in range(self.summary_tab.summary_table.columnCount()):
                item = self.summary_tab.summary_table.item(row, col)
                row_data.append(item.text() if item else "")
            summary_data.append(row_data)
        archive_data["summary"] = summary_data
    ...
    def save_archive(self):
        try:
            ...
            if not os.path.exists(ARCHIVE_FILE):
                ...
                # 确保已有文件具有正确权限
                os.chmod(ARCHIVE_FILE, 0o666)  # 设置读写权限
    ...
    def load_archive(self):
        ...
        try:
            ...
            # 创建备份并返回空数据
            backup_path = ARCHIVE_FILE + ".bak"
            try:
                os.rename(ARCHIVE_FILE, backup_path)
            except Exception as be:
                logging.error(f"备份失败：{str(be)}")
    ...
    def add_new_tab(self):
        ...
        for idx in used_indices:
            if idx == next_index:
                ...
                break
        # 创建新标签页并设置名称
        new_tab = DataEncoderModule(tabmodule=self.tabmodule_widget)
    ...
    def restore_from_archive(self):
        ...
        # 恢复视频
        video_path = archive_data.get("video")
        if video_path and os.path.exists(video_path):
            self.video_player.load_video(video_path)
            self.video_player.file_list.addItem(video_path)

        # 清空现有编码个体标签页（除了数据汇总）
        for i in reversed(range(self.tabmodule_widget.count())):
            widget = self.tabmodule_widget.widget(i)
            if isinstance(widget, DataEncoderModule) and widget is not self.summary_tab:
                self.tabmodule_widget.removeTab(i)

        # 恢复编码个体
        for tab_data in archive_data.get("tabs", []):
            tab_name = tab_data["name"]
            if tab_name == "数据汇总":
                continue
                
            self.add_new_tab()
            new_tab = self.tabmodule_widget.widget(self.tabmodule_widget.count() - 2)
            self.tabmodule_widget.setTabText(self.tabmodule_widget.count() - 2, tab_name)
            new_tab.from_dict(tab_data["data"])

        # 恢复汇总数据
        summary_data = archive_data.get("summary", [])
        if summary_data:
            self.summary_tab.update_table_with_data(summary_data)
            self.summary_tab.generate_summary_data()

        # 如果没有存档数据，则添加一个新的“编码个体1”标签页
        if not archive_data.get("tabs"):
            self.add_new_tab()
    def check_and_restore_archive(self):
        """检查并恢复存档"""
...
class SummaryTab(QWidget):
    ...
    def update_table_with_data(self, data):
        """兼容 DataEncoderModule 的 update_table_with_data 方法"""
        if not data:
            self.summary_table.setRowCount(0)
            return
        ...
        self.summary_table.setColumnCount(column_count)
...
class DataEncoderModule(QWidget):
    def __init__(self, group_name="", tabmodule=None):
        super().__init__()
    ...
    def initUI(self):
        ...
        self.import_btn = QPushButton('导入框架')
        self.import_btn.setFont(QFont('黑体', 10))
        self.import_btn.setFixedSize(90, 30)
        self.import_btn.setStyleSheet("background-color: #a6baff;")
        self.import_btn.clicked.connect(self.import_encoding_framework)
    ...
    def to_dict(self):
        ...
        data = {
            ...
            "time_unit": self.time_unit_entry.text(),
            "startmin": self.startmin.text(),
            "startsec": self.startsec.text(),
            "comment": self.comment_entry.text(),  # 保存注释输入框内容
            "row_number": self.row_number_entry.text(),  # 保存行号输入框内容
            "confirm_enabled": self.confirm_btn.isEnabled(),
            "continue_enabled": self.continue_btn.isEnabled(),
            "undo_enabled": self.undo_btn.isEnabled(),
            "clear_enabled": self.clear_btn.isEnabled(),
            "output_enabled": self.output_btn.isEnabled(),
            "insert_enabled": self.insert_btn.isEnabled(),
            "delete_enabled": self.delete_btn.isEnabled(),
            "time_unit_enabled": self.time_unit_entry.isEnabled(),
            "startmin_enabled": self.startmin.isEnabled(),
            "startsec_enabled": self.startsec.isEnabled(),
            "row_number_enabled": self.row_number_entry.isEnabled()
                }
        data["table_data"] = []
        # 保存表格数据
        ...
        # 保存表格列名
        data["table_headers"] = [self.result_table.horizontalHeaderItem(col).text() for col in range(self.result_table.columnCount())]

        for module in self.modules:
            module_data = {
                "name": module.name,
                "codes": [block['entry'].text() for block in module.blocks]
            }
    ...
    def from_dict(self, data):
        ...
        for module_data in data.get("modules", []):
            self.add_module()
            new_module = self.modules[-1]
            new_module.name = module_data["name"]
            new_module.label.setText(new_module.name)
            new_module.edit_entry.setText(new_module.name)

            while new_module.blocks:
                new_module.remove_block()
                
            for code in module_data.get("codes", []):
                new_module.add_block()
                new_module.blocks[-1]['entry'].setText(code)

        self.startmin.setText(data.get("startmin", "0"))
        self.startsec.setText(data.get("startsec", "0"))
        self.time_unit_entry.setText(data.get("time_unit", "1"))
        self.comment_entry.setText(data.get("comment", ""))  # 恢复注释输入框内容
        self.row_number_entry.setText(data.get("row_number", "1"))  # 恢复行号输入框内容

        self.selected_data = data.get("selected_data", [])
        self.update_table()

        # 恢复按钮状态
        self.confirm_btn.setEnabled(data.get("confirm_enabled", False))
        self.continue_btn.setEnabled(data.get("continue_enabled", False))
        self.undo_btn.setEnabled(data.get("undo_enabled", False))
        self.clear_btn.setEnabled(data.get("clear_enabled", False))
        self.output_btn.setEnabled(data.get("output_enabled", False))
        self.insert_btn.setEnabled(data.get("insert_enabled", False))
        self.delete_btn.setEnabled(data.get("delete_enabled", False))

        # 恢复控件状态（是否可编辑）
        self.time_unit_entry.setEnabled(data.get("time_unit_enabled", True))
        self.startmin.setEnabled(data.get("startmin_enabled", True))
        self.startsec.setEnabled(data.get("startsec_enabled", True))
        self.row_number_entry.setEnabled(data.get("row_number_enabled", False))

        # 确保时间单位、起始时间可以被修改，行号、插入、删除不可操作（根据存档状态）
        if not data.get("confirm_enabled", False):
        ...
        # 恢复表格列名
        table_headers = data.get("table_headers", ["序号", "时间", "维度1", "维度2", "注释"])
        self.result_table.setColumnCount(len(table_headers))
    ...
    def import_encoding_framework(self):
        source_tabs = []
        if self.tabmodule:
            for i in range(self.tabmodule.count()):
                widget = self.tabmodule.widget(i)
                if widget is self:
                    continue
                if isinstance(widget, DataEncoderModule) and len(widget.modules) > 0:
                    tab_text = self.tabmodule.tabText(i)
                    source_tabs.append((i, tab_text, widget))

        # 若有可复制的标签页
        if source_tabs:
            items = [tab_text for _, tab_text, _ in source_tabs]
            ...
            if ok and item:
                ...
                source_encoder = source_tabs[selected_index][2]
                self.copy_modules(source_encoder)  # 执行复制
                QMessageBox.information(self, "成功", f"已从 '{item}' 复制框架！")
                return  # 跳过后续 Excel 导入
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择编码框架文件", "", "Excel Files (*.xlsx);;All Files (*)"
        )
        if not file_path:
            return
        ...
        try:
            ...
            df['维度'] = df['维度'].astype(str)
            df['类型'] = df['类型'].astype(str)

            # 验证列名
            if list(df.columns) != ['维度', '类型']:
                raise ValueError("Excel 文件必须包含 '维度' 和 '类型' 两列")
            ...
            for _, row in df.iterrows():
                ...
                if dim_name != current_dimension:
                    if current_dimension is not None:
                        dimension_sequence.append({'name': current_dimension, 'codes': current_codes})
            ...
            # 创建模块
            for item in dimension_sequence:
                dimension_name = item['name']
                codes = item['codes']
                self.add_module()
                new_module = self.modules[-1]
                new_module.name = str(dimension_name)
                new_module.label.setText(new_module.name)
                new_module.edit_entry.setText(new_module.name)

                for code in codes:
                    new_module.add_block()
                    new_module.blocks[-1]['entry'].setText(str(code))

            QMessageBox.information(self, "成功", "编码框架已成功导入！")

        except Exception as e:
            QMessageBox.critical(self, "错误", f"导入失败：{str(e)}")
    def copy_modules(self, source_encoder):
        """从指定的 DataEncoderModule 复制框架数据"""
        # 清除当前模块
        while self.modules:
            module = self.modules.pop()
            module.setParent(None)
        ...
        # 逐个复制模块
        for source_module in source_encoder.modules:
            self.add_module()
            new_module = self.modules[-1]

            # 设置模块名称
            new_module.name = source_module.name
            new_module.label.setText(new_module.name)
            new_module.edit_entry.setText(new_module.name)
            ...
            # 复制编码块
            for block_info in source_module.blocks:
                code_text = block_info['entry'].text().strip()
                if code_text:
                    new_module.add_block()
                    new_module.blocks[-1]['entry'].setText(code_text)
...
class Module(QWidget):
    ...
    def initUI(self):
        ...
        imagepath=os.path.join(os.path.dirname(__file__), 'data/downarrow(1).png')
...
if __name__ == '__main__':
    ...
    logoimage_path = os.path.join(current_dir, 'data/logo-1种尺寸.png')
    window.setWindowIcon(QIcon(logoimage_path))  # 修改为相对路径
    window.check_and_restore_archive()  # 新增
    window.show()
    sys.exit(app.exec_())
